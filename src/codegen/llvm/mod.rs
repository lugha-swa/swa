...
fn lower_function(
    ...
    unsafe {
        ...
        if func.c_abi {
            LLVMSetFunctionCallConv(llvm_func, 0); // 0 = LLVMCCallConv
        }
        ...
        if func.sret_value_id.is_some() {
            let sret_kind =
                LLVMGetEnumAttributeKindForName(c_str("sret").as_ptr(), 4);
            if sret_kind != 0 {
                // Kigezo cha kwanza ni *Muundo — toa aina ya ndani.
                let struct_llvm_ty = match &func.params[0].1 {
